...
class DDQN:
    ...
    def replay(self):
        ...
        actions_network = np.argmax(self.network.predict_on_batch(next_states), axis=1)
        values_target = self.target_network.predict_on_batch(next_states)
        result = np.zeros(self.batch_size)
        for i in range(self.batch_size):
            result[i] = values_target[i, actions_network[i]]

        targets = rewards + self.gamma * result * (1 - dones)
